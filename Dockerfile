...
RUN go build -ldflags "-X $GO_PACKAGE/pkg/version.versionFromGit=$(git describe --long --tags --abbrev=7 --match 'v[0-9]*' 2>/dev/null || echo 'unknown')" ./cmd/cloud-credential-operator
RUN go build -ldflags "-X $GO_PACKAGE/pkg/version.versionFromGit=$(git describe --long --tags --abbrev=7 --match 'v[0-9]*' 2>/dev/null || echo 'unknown')" ./cmd/ccoctl
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GO_COMPLIANCE_POLICY=exempt_all \
    go build -ldflags "-X $GO_PACKAGE/pkg/version.versionFromGit=$(git describe --long --tags --abbrev=7 --match 'v[0-9]*' 2>/dev/null || echo 'unknown')" \
    ./cmd/cloud-credential-tests-ext && \
    mkdir -p /tmp/build && \
    cp ./cloud-credential-tests-ext /tmp/build/cloud-credential-tests-ext && \
    gzip /tmp/build/cloud-credential-tests-ext
