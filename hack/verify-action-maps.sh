...
#
# If RELEASE_IMAGE is omitted, the script pulls the latest 4.17.0 nightly.
...
if [[ -z "$RELEASE_IMAGE" ]]; then
  ...
  RELEASE_IMAGE=$(curl -sfL 'https://amd64.ocp.releases.ci.openshift.org/api/v1/releasestream/4.17.0-0.nightly/latest' | jq -r '.pullSpec // empty') || true
  if [[ -z "$RELEASE_IMAGE" ]]; then
