...
data:
  clouds.yaml: ` + badCreds
	...
	g.It("Author:fxie-Critical-64885-CCO-based flow for olm managed operators and AWS STS", func() {
		...
		var (
			testDataDir      = compat_otp.FixturePath("testdata", "cluster_operator/cloudcredential")
			testCaseID       = "64885"
...
var _ = g.Describe("[sig-cco][OTP][Level0] Cluster_Operator CCO is enabled", func() {
	...
	g.It("NonHyperShiftHOST-Author:mihuang-Critical-33204-[cco-passthrough]IPI on azure with cco passthrough mode", func() {
	...
	g.It("NonHyperShiftHOST-ROSA-OSD_CCS-Author:jshu-Critical-36498-CCO credentials secret change to STS-style", func() {
		//Check IAAS platform type
		iaasPlatform := compat_otp.CheckPlatform(oc)
		if iaasPlatform != "aws" {
...
var _ = g.Describe("[sig-cco][OTP] Cluster_Operator CCO is disabled", func() {
	defer g.GinkgoRecover()
