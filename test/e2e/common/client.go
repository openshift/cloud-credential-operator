...
func NewCLI(project, adminConfigPath string) *CLI {
	...
	client.kubeFramework = NewFrameworkNoBeforeEach(project, e2e.Options{}, nil)
...
func (c *CLI) Outputs() (string, string, error) {
	...
	case *exec.ExitError:
		e2e.Logf("Error running %v:\nStdOut>\n%s\nStdErr>\n%s\n", cmd, stdOut, stdErr)
		return stdOut, stdErr, &ExitError{ExitError: err, Cmd: c.execPath + " " + strings.Join(c.finalArgs, " "), StdErr: stdErr}
	default:
		FatalErr(fmt.Errorf("unable to execute %q: %v", c.execPath, err))
		// unreachable code
		return "", "", nil
...
var PureFrameworkExtensions = []func(f *e2e.Framework){}

// NewFrameworkNoBeforeEach creates a test framework but does NOT register any Ginkgo nodes.
func NewFrameworkNoBeforeEach(baseName string, options e2e.Options, client kubernetes.Interface) *e2e.Framework {
	f := &e2e.Framework{
		BaseName:  baseName,
		Options:   options,
		ClientSet: client,
	...
	for _, extension := range PureFrameworkExtensions {
		extension(f)
	}
	return f
}

// NewCLIWithoutGinkgo initialize the upstream E2E framework and set the namespace to match
// with the project name. Note that this function does not initialize the project
// role bindings for the namespace and does not register any Ginkgo nodes.
func NewCLIWithoutGinkgo(project, adminConfigPath string) *CLI {
	client := &CLI{}
