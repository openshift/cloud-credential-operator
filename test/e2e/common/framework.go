...
func DumpPodsCommand(c kubernetes.Interface, ns string, selector labels.Selector, cmd string) {
	...
	for name, stdout := range values {
		...
		e2e.Logf("%s", name+": "+strings.Join(strings.Split(stdout, "\n"), fmt.Sprintf("\n%s: ", name)))
	}
}
	...
	for {
		...
		for {
			...
			if e, ok := val.Object.(*imagev1.ImageStream); ok {
				...
				if isFailed(e) {
					return fmt.Errorf("the image stream %q status is %q", name, e.Annotations[imagev1.DockerImageRepositoryCheckAnnotation])
				}
			}
		}
	}
