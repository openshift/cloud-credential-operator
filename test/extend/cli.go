...
func isHyperShiftCluster(oc *CLI) bool {
	controlPlaneTopology, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("infrastructure", "cluster", "-o=jsonpath={.status.controlPlaneTopology}").Output()
	if err != nil {
		return false
	}
	return strings.TrimSpace(controlPlaneTopology) == "External"
}

// skipIfHyperShift skips the test if the cluster has an externally hosted control plane
func skipIfHyperShift(oc *CLI) {
	if isHyperShiftCluster(oc) {
		g.Skip("Test is skipped on externally hosted control plane clusters (e.g., HyperShift)")
	}
}

// skipIfNotHyperShift skips the test if the cluster does NOT have an externally hosted control plane
func skipIfNotHyperShift(oc *CLI) {
	if !isHyperShiftCluster(oc) {
		g.Skip("Test is skipped on non-externally-hosted control plane clusters")
	}
}
