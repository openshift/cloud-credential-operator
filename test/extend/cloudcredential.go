...
data:
  clouds.yaml: ` + badCreds
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:50869] NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Medium-High-53283-High-77285-CCO Pod Security Admission change", ote.Informing(), func() {
		...
		containerCountStr, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("pod", ccoPodName, "-n", DefaultNamespace, "-o=jsonpath={.spec.containers[*].name}").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		containerCount := len(strings.Fields(containerCountStr))
		...
		o.Expect(dropAllCount).To(o.Equal(containerCount))
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:34470] NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Critical-Cloud credential operator health check", ote.Informing(), func() {
		...
		imagePullPolicy, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("deployment", "cloud-credential-operator", "-n", DefaultNamespace, "-o=jsonpath={.spec.template.spec.containers[?(@.name==\"cloud-credential-operator\")].imagePullPolicy}").Output()
		o.Expect(err).NotTo(o.HaveOccurred())
