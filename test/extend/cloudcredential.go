...
var _ = g.Describe(`[sig-cco][Jira:"Cloud Credential Operator"]`, func() {
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:23352] Cluster_Operator CCO is enabled NonHyperShiftHOST-PstChkUpgrade-NonPreRelease-Author:mihuang-High-23352-Cloud credential operator resets progressing transition timestamp when it upgrades", func() {
		skipIfHyperShift(oc)
	...
	// so adding [Slow]
	g.It("[Suite:cco/disruptive][OTP][PolarionID:31768][Disruptive][Serial][Slow]Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:lwan-High-31768-Report the mode of cloud-credential operation as a metric", func() {
		skipIfHyperShift(oc)
...
spec:
  credentialsMode: ` + originCCOMode
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:33204][LEVEL0][platform:azure] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:mihuang-LEVEL0-Critical-33204-[cco-passthrough]IPI on azure with cco passthrough mode", func() {
		skipIfHyperShift(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:45415][LEVEL0][Disruptive][Serial][platform:openstack] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:lwan-High-45415-[Bug 1940142] Reset CACert to correct path", func() {
		skipIfHyperShift(oc)
...
data:
  clouds.yaml: ` + badCreds
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:36498][LEVEL0][platform:aws] Cluster_Operator CCO is enabled NonHyperShiftHOST-ROSA-OSD_CCS-Author:jshu-LEVEL0-Critical-36498-CCO credentials secret change to STS-style", func() {
		skipIfHyperShift(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:50869] Cluster_Operator CCO is enabled Author:jshu-NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Medium-50869-High-53283-High-77285- CCO Pod Security Admission change", func() {
		skipIfHyperShift(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:80542] Cluster_Operator CCO is enabled Author:mihuang-NonHyperShiftHOST-ROSA-OSD_CCS-ARO-High-80542- Enable readOnlyRootFilesystem on all containers", func() {
		skipIfHyperShift(oc)
	...
	g.It("[Suite:cco/disruptive][OTP][PolarionID:48360][Disruptive][Serial][platform:aws] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:jshu-Medium-48360-Reconciliation of aws pod identity mutating webhook did not happen", func() {
		skipIfHyperShift(oc)
		//Check IAAS platform type
		iaasPlatform := checkPlatform(oc)
		if iaasPlatform != "aws" {
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:45975][Disruptive][Serial] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:jshu-Medium-45975-Test cco condition changes", func() {
		skipIfHyperShift(oc)
		//Check CCO mode
		mode, err := getCloudCredentialMode(oc)
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:45219] Cluster_Operator CCO is enabled NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Author:jshu-High-45219-A fresh cluster should not have stale CR", func() {
		skipIfHyperShift(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:34470] Cluster_Operator CCO is enabled NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Author:jshu-Critical-34470-Cloud credential operator health check", func() {
		skipIfHyperShift(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:66538][LEVEL0][platform:azure] Cluster_Operator CCO is enabled NonHyperShiftHOST-OSD_CCS-ARO-Author:mihuang-LEVEL0-Critical-66538-Azure workload identity cluster healthy check.", func() {
		skipIfHyperShift(oc)
		mode, _ := getCloudCredentialMode(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:64885][platform:aws] Cluster_Operator CCO is enabled Author:fxie-Critical-64885-CCO-based flow for olm managed operators and AWS STS", func() {
...
spec:
  cloudTokenPath: ` + cloudTokenPath
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:69971][platform:azure] Cluster_Operator CCO is enabled NonHyperShiftHOST-OSD_CCS-ARO-Author:jshu-Critical-69971-Azure workload identity management for olm managed operators", func() {
		skipIfHyperShift(oc)
		skipIfPlatformTypeNot(oc, "azure")
...
spec:
  ...
  providerSpec:
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:75429][platform:gcp] Cluster_Operator CCO is enabled Author:jshu-NonHyperShiftHOST-OSD_CCS-Critical-75429-GCP workload identity management for olm managed operators", func() {
		skipIfHyperShift(oc)
...
var _ = g.Describe(`[sig-cco][Jira:"Cloud Credential Operator"]`, func() {
	...
	*/
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:68220] Cluster_Operator CCO is disabled NonHyperShiftHOST-Author:fxie-Critical-68220-Leverage Composable OpenShift feature to make cloud-credential optional", func() {
		skipIfHyperShift(oc)
		var (
