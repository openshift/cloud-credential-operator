...
var _ = g.Describe("[sig-cco][Jira:Cloud Credential Operator]", func() {
	...
	// so adding [Slow]
	g.It("[Suite:cco/disruptive][OTP][PolarionID:31768][Disruptive] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:lwan-High-31768-Report the mode of cloud-credential operation as a metric [Slow]", func() {
...
spec:
  credentialsMode: ` + originCCOMode
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:33204][LEVEL0] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:mihuang-LEVEL0-Critical-33204-[cco-passthrough]IPI on azure with cco passthrough mode", func() {
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:45415][LEVEL0] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:lwan-High-45415-[Bug 1940142] Reset CACert to correct path [Disruptive]", func() {
...
data:
  clouds.yaml: ` + badCreds
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:36498][LEVEL0] Cluster_Operator CCO is enabled NonHyperShiftHOST-ROSA-OSD_CCS-Author:jshu-LEVEL0-Critical-36498-CCO credentials secret change to STS-style", func() {
	...
	g.It("[Suite:cco/disruptive][OTP][PolarionID:48360][Disruptive] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:jshu-Medium-48360-Reconciliation of aws pod identity mutating webhook did not happen", func() {
		//Check IAAS platform type
		iaasPlatform := compat_otp.CheckPlatform(oc)
		if iaasPlatform != "aws" {
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:45975][Disruptive] Cluster_Operator CCO is enabled NonHyperShiftHOST-Author:jshu-Medium-45975-Test cco condition changes", func() {
		//Check CCO mode
		mode, err := getCloudCredentialMode(oc)
		o.Expect(err).NotTo(o.HaveOccurred())
		e2e.Logf("cco mode in cluster is %v", mode)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:66538][LEVEL0] Cluster_Operator CCO is enabled NonHyperShiftHOST-OSD_CCS-ARO-Author:mihuang-LEVEL0-Critical-66538-Azure workload identity cluster healthy check.", func() {
		mode, _ := getCloudCredentialMode(oc)
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:64885] Cluster_Operator CCO is enabled Author:fxie-Critical-64885-CCO-based flow for olm managed operators and AWS STS", func() {
...
spec:
  cloudTokenPath: ` + cloudTokenPath
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:69971] Cluster_Operator CCO is enabled NonHyperShiftHOST-OSD_CCS-ARO-Author:jshu-Critical-69971-Azure workload identity management for olm managed operators", func() {
		compat_otp.SkipIfPlatformTypeNot(oc, "azure")
...
spec:
  ...
  providerSpec:
	...
	g.It("[Suite:cco/conformance/parallel][OTP][PolarionID:75429] Cluster_Operator CCO is enabled Author:jshu-NonHyperShiftHOST-OSD_CCS-Critical-75429-GCP workload identity management for olm managed operators", func() {
