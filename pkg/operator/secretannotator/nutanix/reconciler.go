...
// NewReconciler will return a reconciler for handling Nutanix cloud cred secrets.
...
func Add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = secretutils.WatchCCOConfig(c, types.NamespacedName{
		...
		Name:      constants.NutanixCloudCredSecretName,
	})
	if err != nil {
		return err
	}
...
func cloudCredSecretObjectCheck(secret metav1.Object) bool {
	return secret.GetNamespace() == constants.CloudCredSecretNamespace && secret.GetName() == constants.NutanixCloudCredSecretName
}

// Reconcile handles annotating the cloud cred secret.
func (r *ReconcileCloudCredSecret) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	start := time.Now()
...
func (r *ReconcileCloudCredSecret) validateCloudCredsSecret(secret *corev1.Secret) error {
	...
	// TODO: Check for any expected keys.
