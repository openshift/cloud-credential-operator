...
func deleteOIDCObjectsFromBucket(client aws.Client, bucketName, namePrefix string) error {
	...
	for {
		...
		if !*objectsMetadata.IsTruncated {
			break
		}
		nextMarker = objectsMetadata.NextMarker
	}
...
func deleteCloudFrontOriginAccessIdentity(client aws.Client, namePrefix string) error {
	...
	for {
		...
		if !*listCloudFrontOriginAccessIdentitiesOutput.CloudFrontOriginAccessIdentityList.IsTruncated {
			break
		}
		nextMarker = listCloudFrontOriginAccessIdentitiesOutput.CloudFrontOriginAccessIdentityList.NextMarker
	}
	return nil
}
...
func deleteCloudFrontDistribution(client aws.Client, namePrefix string) error {
	...
	for {
		...
		if !*ListCloudFrontDistributionsOutput.DistributionList.IsTruncated {
			break
		}
		nextMarker = ListCloudFrontDistributionsOutput.DistributionList.NextMarker
	}
	return nil
}
...
func deleteIAMRoles(client aws.Client, namePrefix string) error {
	...
	for {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to fetch a list of IAM roles, pagination marker: %v", nextMarker)
		}
