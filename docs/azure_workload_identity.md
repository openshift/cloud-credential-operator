...
8. Configure `install-config.yaml` to enable the tech preview featureset.
...
10. Copy the manifests created in the step 4 and put them in the install `manifests` directory generated by `openshift-install create manifests`.

   ```
   cp ccoctl_output_dir/manifests/* /path/to/dir/with/install/manifests/
   ```

11. Copy the private key for the ServiceAccount signer and put it in the same location as `install-config.yaml`.
